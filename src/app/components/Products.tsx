...
function Hit({ hit }: any) {
  ...
  cleanName = cleanName.replace(sizeRegex, "").replace(emptyParensRegex, "").trim();
  ...
  return (
    <div className="p-4 border border-gray-300 rounded-lg shadow-sm hover:shadow-md transition bg-white flex flex-col justify-between h-[320px]">
      ...
      <button
        ...
        className="mt-3 px-3 py-2 text-sm rounded bg-green-600 text-white hover:bg-green-700 transition"
      >
        Add to Cart
      </button>
    </div>
...
export default function Products({ searchTerm }: { searchTerm: string }) {
  return (
    <InstantSearch indexName="skincare_products_csv" searchClient={searchClient}>
